...
class log():
    def __init__(self, details, fileName='log.txt'):
        ...
        header = ','.join(
            ['Iteration', 'Alice-Bob/Loss', 'EveLoss', 'EveIncorrect', 'Alice/Bob-Incorrect', 'RunNumber'])
...
def getBobAliceLoss(bob, eve, alice, messageLength):
    ...
    totalLoss = conversationLoss + (7.5 * snoopLoss)
    return totalLoss
...
def getData(numBits, batchSize):
    ...
    queue = []
    getBitSequence = lambda numBits, BatchSize: queue.insert(0, np.random.randint(2, size=(batchSize, numBits)).astype(float))
